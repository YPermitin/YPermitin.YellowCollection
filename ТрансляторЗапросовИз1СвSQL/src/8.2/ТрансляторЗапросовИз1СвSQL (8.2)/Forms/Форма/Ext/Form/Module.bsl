...
Процедура КонструкторЗапроса1С(Команда)
	...
	Если КонструкторЗапроса.ОткрытьМодально() Тогда		
		Объект.ТекстЗапроса1С = КонструкторЗапроса.Текст;		
		ОпределитьПараметрыЗапроса();		
	КонецЕсли;
	#КонецЕсли
...
Процедура СохранитьПланЗапросаXML(Команда)
	...
	Если ДиалогСохраненияФайла.Выбрать() Тогда
		ПутьФайлаДляСохранения = ДиалогСохраненияФайла.ПолноеИмяФайла;			
...
Процедура ОткрытьПланЗапроса(Команда)
	...
	ЗапуститьПриложение(ВременныйФайлСПланом);
...
Функция ВыполнитьЗапрос1СНаСервере()
	...
	Попытка
		...
		// ЗапросДляИдентификацииСоединенияПоВременнойТаблице
...
Процедура ОпределитьПараметрыЗапроса()
	...
	Попытка
		СхемаЗапроса = Новый Запрос;
		СхемаЗапроса.Текст(Объект.ТекстЗапроса1С);		
